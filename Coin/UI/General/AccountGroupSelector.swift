...
struct AccountGroupSelector: View {
    ...
    @AppStorage("selectedAccountGroupID") var selectedAccountGroupID: String = ""
    ...
    var body: some View {
        ...
        .task {
            do {
                try await vm.load()
                if let selectedAccountGroup = vm.accountGroups.first(where: { $0.id == UUID(uuidString: selectedAccountGroupID)}) {
        ...
        .onChange(of: selectedAccountGroup.selectedAccountGroup) { _, newValue in
            selectedAccountGroupID = newValue.id.uuidString
        }
    }
}
